...
res = read_f06('MA_final/referenz/sol145/dlm/sol145_DLM_025.f06')
pages = join_flutter_pages(res.flutter)
df = flutter_pages_to_df(pages)
get_critical_roots(df)

p = plot_vf_vg(df,modes=(1, 3, 4, 5, 7, 8, 10, 12)) # 4, 7, 12how many modes want to print, if you want to all, del modes=()
plt.title('FLUTTER ANALYSIS : Ref2020_DLM_025', pad=160)
plt.xlabel('Equivalent Airspeed (m/s)')
plt.ylabel('                                                      DAMPING                       FREQUENCY(Hz)')
plt.savefig('MA_final/referenz/sol145/dlm/Ref2020_DLM_025_vgvf.png')
plt.show()

p = plot_complex(df, modes=(1, 3, 4, 5, 7, 8, 10, 12))
plt.axvline(x=0, ymin=0, ymax=1, linestyle='dashed')
plt.title('FLUTTER_Root_Locus_Method : Ref2020_DLM_025')
plt.xlabel('Real Part (rad/sec)')
plt.ylabel('Imaginary part (Hz)')
plt.savefig('MA_final/referenz/sol145/dlm/Ref2020_DLM_025_root.png')
