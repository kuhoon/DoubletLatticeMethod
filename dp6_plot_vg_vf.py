...
res = read_f06('MA_final/DP6/sol145/strip/sol145_strip_dp6_m1.f06')
...
plt.title('FLUTTER ANALYSIS : Ref2020_Strip_DP6_M1', pad=160)
plt.xlabel('Equivalent Airspeed (m/s)')
plt.ylabel('                                                      DAMPING                       FREQUENCY(Hz)')
plt.savefig('MA_final/DP6/sol145/strip/Ref2020_strip_DP6_M1_vgvf.png')
plt.show()

p = plot_complex(df, modes=[1, 3, 4, 6, 7, 8, 9, 12])
plt.axvline(x=0, ymin=0, ymax=1, linestyle='dashed')
plt.title('FLUTTER_Root_Locus_Method : Ref2020_Strip_DP6_M1')
plt.xlabel('Real Part (rad/sec)')
plt.ylabel('Imaginary part (Hz)')
plt.savefig('MA_final/DP6/sol145/strip/Ref2020_strip_DP6_M1_root.png')
