...
model_path = os.path.join(cw_path, 'MA_final/referenz/sol103/ungekoppelt/sol103_025_uncoupled.op2')
...
x_data = np.delete(sol103.eigenvectors[1].modes,[1, 5, 8, 10])
y_data = np.delete(sol103.eigenvectors[1].mode_cycles, [1, 5, 8, 10])
colors = ['r'] * len(x_data)  # 기본 색상은 파란색으로 설정
colors[0] = 'b'  # 1번 요소 막대는 빨간색으로 변경
colors[1] = 'b'  # 1번 요소 막대는 빨간색으로 변경
# colors[2] = 'b'  # 1번 요소 막대는 빨간색으로 변경
colors[3] = 'b'  # 6번 요소 막대는 빨간색으로 변경
# colors[4] = 'g'  # 6번 요소 막대는 빨간색으로 변경
colors[5] = 'b'  # 6번 요소 막대는 빨간색으로 변경
# colors[6] = 'g'  # 6번 요소 막대는 빨간색으로 변경
colors[7] = 'b'  # 6번 요소 막대는 빨간색으로 변경
...
plt.title('SOL103_025 Mode Frequency')
plt.grid()
...
plt.savefig('MA_final/referenz/sol103/ungekoppelt/sol103_025_uncoupled.png')
plt.show()